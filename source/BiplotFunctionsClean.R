...
{  
   ...
   list(col = col, pch = pch,bg = bg, cex = cex, label = label, label.cex = label.cex, label.side = label.side, 
...
# biplot.ax.control <- function (p, X.names, which = 1:p, type = "prediction", col = "darkolivegreen", lwd = 1, lty = 1, 
...
biplot.ax.control <- function (p, X.names, which = 1:p, type = "prediction", col = "darkolivegreen", lwd = 1, lty = 1,
                               label = "Orthog", label.col = "black", label.cex = 0.75, label.dist = 0, ticks = 5,
                               tick.col = "black", tick.size = 1, tick.label = T, tick.label.col = tick.col, tick.label.cex = 0.6,
                               tick.label.side = "left", tick.label.offset = 0.5, tick.label.pos = 1,
                               predict.col = col, predict.lwd = lwd, predict.lty = lty, ax.names = X.names,
                               orthogx = 0, orthogy = 0, oblique = NULL)
...
biplot.ordinal.ax.control <- function (p, X.names, num.levels, which = 1:p, col = "darkolivegreen", lwd.factor = 1.5, lty = 1, 
...
biplot.kappa.ellipse.control <- function (J, ellipse.names, df=2, which = NULL, kappa = NULL, alpha = 0.95, 
                                          col = c(1:8, 3:1), lty = 1, lwd = 1, alpha.transparency = 0.5) 
...
biplot.density.1D.control <- function (J, class.names, which = NULL, bw = "nrd0", kernel = "gaussian", col = c(1:8, 3:1), 
                                       lty = 1, lwd = 1) 
{  if (!all(is.numeric(which))) which <- match(which, class.names, nomatch = 0)
   which <- which[which <= J]
   which <- which[which > 0]
...
{  r <- length(dims)
   ...
   diag(Jmat)[dims] <- 1
#   if (nrow(mat)<ncol(mat)) Jmat <- diag(nrow(mat))
...
{  ax.num <- ax.which[j]
   tick <- ax.tickvec[j]
   if(!is.null(nrow(axes.rows))){
      ax.direction <- axes.rows[ax.num, ]  
   }
   else{
      axes.rows<-t(as.matrix(axes.rows))
      ax.direction <- axes.rows[ax.num, ]  
   }
   r <- ncol(axes.rows)
...
biplot.create.regions2 <- function (Z,Z_true,region.style, plot.range=NULL, col=NULL, add=TRUE, show.points=FALSE, plot=TRUE, plot.coords=NULL, border=c("black",NA), eps=0.00001, ...)
{
   ...
   J <- nrow(Z)
   usr <- apply(Z_true*1.2, 2, range)*1.5
   plot.range <- c(min(usr[1, ]), max(usr[2, ]))
   #plot.range <- 1*c(min(par("usr")[c(1,3)]),max(par("usr")[c(2,4)]))
          ...
          for (j in 1:length(classes)) 
            { class.num <- classes[j]
              ...
              if(nrow(Z.class)>0) for (i in 1:nrow(Z.class)) points(x = Z.class[i, 1], y = Z.class[i, 2], pch = sample.style$pch[j], 
                                                                    col = sample.style$col[j], cex = sample.style$cex[j],bg=sample.style$bg[j])                  
            }
}
...
draw.biplot.1D <- function (Z, G = matrix(1, nrow = nrow(Z), ncol = 1), classes = "all", Z.means = NULL, z.axes = NULL, 
                            z.bags = NULL, z.ellipse = NULL, Z.new = NULL, Z.density = NULL, sample.style, mean.style = NULL, 
                            ax.style = NULL, bag.style = NULL, ellipse.style = NULL, new.sample.style = NULL, 
...
.lin.axes.plot <- function(z.axes, ax.style, predict.mat) {
          for (i in 1:length(ax.style$which)) 
             {  ax.num <- ax.style$which[i]
                ...
                invals <- x.vals < usr[2] & x.vals > usr[1]
                std.markers <- zapsmall(marker.mat[invals, 2])
                x.vals <- x.vals[invals]
                if (ax.style$tick.label[i]) label.on.off <- rep(1, sum(invals)) else rep(0, sum(invals))
                if (!ax.style$tick.label[i]) label.on.off[c(1, length(label.on.off))] <- 1
...
.density.plot <- function(Z.density, density.style) {
   ...
   plot(range(Z) * exp.factor, c(max.y, -length(z.axes) - 1), xlim = range(Z * exp.factor), xaxt = "n", yaxt = "n", xlab = "", ylab = "", type = "n")
   usr <- par("usr")
   if (!is.null(z.axes)) .lin.axes.plot(z.axes, ax.style, predict.mat)
...
draw.biplot.3D <- function (Z, G = matrix(1, nrow = nrow(Z), ncol = 1), classes = "all", Z.means = NULL, z.axes = NULL, z.bags = NULL, z.ellipse = NULL, Z.new = NULL, sample.style, mean.style = NULL, ax.style = NULL, 
...
.predict.func <- function(p.point, coef, col, lty, lwd) {
          if (is.na(coef[2])) 
            ...
            if (coef[2] == 0) lines(rep(p.point[1], 2), p.point[2:3], col = col, lwd = lwd, lty = lty)
            else { intercept.projection <- p.point[2] + p.point[1]/coef[2]
                   project.on.x <- (intercept.projection - coef[1])/(coef[2] + 1/coef[2])
                   project.on.y <- coef[1] + coef[2] * project.on.x
                   lines(c(p.point[1], project.on.x), c(p.point[2], project.on.y), col = col, lwd = lwd, lty = lty)
                      ...
                      {  ax.num <- ax.style$which[i]
                         ...
                         if (is.na(lin.coef[2])) 
                           ...
                           {  y1.ster <- lin.coef[2] * usr[1] + lin.coef[1]
                              ...
                              if (lin.coef[2] > 0) { if (x.vals[1] < x.vals[h]) if (y1.ster <= usr[4] & y1.ster >= usr[3]) mtext(text = ax.style$names[i], side = 2, line = ax.style$label.dist[i], adj = adjust[2], at = y1.ster, col = ax.style$label.col[i], cex = ax.style$label.cex[i])
...
.new.samples.plot <- function(Z.new, new.sample.style) {
   ...
   require(rgl)
   usr <- apply(Z, 2, range) * exp.factor
...
PCAbiplot <- function (X, G = NULL, scaled.mat = FALSE, dim.biplot = c(2, 1, 3), e.vects = 1:ncol(X), 
                       ...
                       legend.type = list(...), legend.format = list(...), 
                       dim3.plane.col = "lightgrey", dim3.xdiameter = 2, dim3.ydiameter = 2, adjust.3d = c(0.5, 0.5), 
                       bag.alpha.3d = 0.7, aspect.3d = "iso", ax.list.3d = "black", cex.3d = 0.6, 
                       col.text.3d = "black", font.3d = 2, predictions.3D = TRUE, size.ax.3d = 0.5, size.means.3d = 10, 
                       size.points.3d = 5, xTitles.3d = c("", "", "Dim 1", "Dim 2", "Dim 3"), 
                       ID.labs = FALSE, ID.3d = 1:nrow(X), 
                       large.scale = FALSE, ort.lty = 1, ...) 
...
CVAbiplot <- function (X, G = NULL, dim.biplot = c(2, 1, 3), e.vects = 1:ncol(X),sample.pch=NULL,region.colours=NULL, correlation.biplot = FALSE, 
                       ...
                       legend.type = list(...), legend.format = list(...), 
                       dim3.plane.col = "lightgrey", dim3.xdiameter = 2, dim3.ydiameter = 2, adjust.3d = c(0.5, 0.5), 
                       bag.alpha.3d = 0.7, aspect.3d = "iso", ax.list.3d = "black", cex.3d = 0.6, col.text.3d = "black", 
                       ...
                       large.scale = FALSE, ort.lty = 1, prior.p = NULL, class.dim=dim.biplot[1], ...) 
{  dim.biplot <- dim.biplot[1]
   ...
   if(!is.null(sample.pch)){
      if(sample.pch[1]>=21){
         samples$col<-gray.colors(J, start = 0.5, end = 0.9, gamma = 2.2, alpha = 1)
         samples$pch<-sample.pch
         temp.style<- samples$col
         samples$col<-samples$bg
         samples$bg<-temp.style
      }
   }
   ...
   if (dim.biplot == 2 & !is.null(prior.p))
     {
       ...
       if(!is.null(region.colours)){
          class.regions$col<-region.colours
       }
...
CATPCAbiplot <- function (X, Xcont=NULL, factor.type=rep("nom", ncol(X)), G = NULL, space.fill=NULL,scaled.mat = FALSE, 
                          ...
                          reflect = c(FALSE, "x", "y"), rotate=NULL, select.origin = FALSE,sample.pch=NULL, 
                          legend.type = list(...), legend.format = list(...), 
                          dim3.plane.col = "lightgrey", dim3.xdiameter = 2, dim3.ydiameter = 2, adjust.3d = c(0.5, 0.5), 
                          bag.alpha.3d = 0.7, aspect.3d = "iso", ax.list.3d = "black", cex.3d = 0.6, col.text.3d = "black", 
                          font.3d = 2, predictions.3D = TRUE, size.ax.3d = 0.5, size.means.3d = 10, size.points.3d = 5, 
...
Yr <- scale(svd.out$u[, e.vects] %*% diag(svd.out$d[e.vects]) %*% t(svd.out$v[, e.vects]), scale = FALSE, center = TRUE)
...
Vr <- Hmat.cent.svd$v[, e.vects]
Z <- Hmat.cent %*% Hmat.cent.svd$v[, e.vects]
...
if (!is.null(Xcont)) 
{ ax <- do.call("biplot.ax.control", c(ncol(Xcont), list(cont.var.names), ax,tick.label.col="black",label.col = "black"))
...
axes.direction <- 1/(diag(Vr.all %*% t(Vr.all))) * Vr.all %*% rotate.mat %*% reflect.mat
axes.direction.nominal <- axes.direction[(1:length(factor.type))[factor.type=="nom"],]
axes.direction.ordinal <- axes.direction[(1:length(factor.type))[factor.type=="ord"],]
axes.direction <- axes.direction[-(1:length(factor.type)),]
...
if (!is.null(ax))
{
   ...
   for (j in 1:length(ax$which))
      z.axes[[j]] <- calibrate.cat.axis(j, axes.direction, ax$which, ax$orthgx, ax$orthogy, ax$oblique, 
...
if (!is.null(ax.nominal))
{
   ...
   for (j in 1:length(ax.nominal$which))
      z.axes.nominal[[j]] <- calibrate.cat.axis(j, axes.direction.nominal, ax.nominal$which, ax.nominal$orthgx, ax.nominal$orthogy, ax.nominal$oblique, 
...
if (!is.null(ax.ordinal))
{
   ...
   z.axes.ordinal[[j]] <- calibrate.cat.axis(j, axes.direction.ordinal, ax.ordinal$which, ax.ordinal$orthgx, ax.ordinal$orthogy, ax.ordinal$oblique, 
...
if (dim.biplot == 2 & !is.null(prior.p))
{
   ...
   if(!is.null(space.fill)){
      class.regions$space.fill<-space.fill
   }
   
   G <- biplot.check.G(G, nrow(Hmat.cent))
   Nmat <- t(G) %*% G
...
if (dim.biplot == 2)
   draw.biplot(
      ...
      Z.means = Z.means.mat,
      z.axes = z.axes,
      z.axes.nominal = z.axes.nominal,
      z.axes.ordinal = z.axes.ordinal,
      z.bags = z.bags,
      z.ellipse = z.ellipse,
      Z.region = Z.region,
      Z.density = z.density,
      sample.style = samples,
      mean.style = class.means,
      ax.style = ax,
      ax.nominal.style = ax.nominal,
      ax.ordinal.style = ax.ordinal,
      bag.style = alpha.bags,
      ellipse.style = kappa.ellipse,
      density.style = density.style,
      region.style = class.regions,
...
if (dim.biplot == 3) draw.biplot.3D(Z = Z, G = G, classes = classes, Z.means = Z.means.mat, z.axes = z.axes, 
                                    ...
                                    ax.style = ax, bag.style = alpha.bags, ellipse.style = kappa.ellipse, 
                                    predict.samples = predict.samples, predict.means = predict.means, 
                                    Title = Title, exp.factor = exp.factor, ...)
...
list (zlist=zlist, H=Hmat.cent,Zmat=Z,RSS=RSS,diff=ep)
}

CVA_H<-
   function (X,
             Xcont = NULL,
             factor.type = rep("nom", ncol(X)),
             G = NULL,
             scaled.mat = FALSE,
             space.fill=NULL,
             h.rank=2,
             ...
             size.means.3d = 10,
             size.points.3d = 5,
             xTitles.3d = c("", "", "Dim 1", "Dim 2", "Dim 3"),
             ID.labs = FALSE,
             ID.3d = 1:nrow(X),
             large.scale = FALSE,
             ort.lty = 1,
             ...
             ...)
   {
      ...
      if (dim.biplot != 1 &
          dim.biplot != 2 &
          dim.biplot != 3)
         stop("Only 1D, 2D and 3D biplots")
      e.vects <- e.vects[1:dim.biplot]
      h.vects<- 1:h.rank
      reflect <- reflect[1]
      ...
      K <- min(p+ncol(Xcont), J - 1)
      if (!is.null(Xcont))
         cont.means <- apply(Xcont, 2, mean)
      else
         cont.means <- NULL
      if (!is.null(Xcont))
         Xcont.cent <- scale(Xcont, scale = scaled.mat)
      else
         Xcont.cent <- NULL
      if (!is.null(Xcont)) {
         if (scaled.mat) {
            cont.sd <- apply(Xcont, 2, sd)
         }
         else {
            cont.sd <- rep(1, ncol(Xcont))
         }
      }
      if (!all(is.numeric(classes)))
         classes <- match(classes, dimnames(G)[[2]], nomatch = 0)
      classes <- classes[classes <= J]
      classes <- classes[classes > 0]
      ...
      N.mat <- diag(n) - matrix(1, nrow = n, ncol = n) / n
      svd.mca <-
         svd(L.mat.min.half %*% t(Gmat) %*% N.mat %*% Gmat %*% L.mat.min.half /
                p)
      z.ini <- svd.mca$v[, 1]
      names(z.ini) <- CatLabels
      count <- 0
      for (k in 1:p) {
         ...
         if (identical(factor.type[k], "ord"))
         {
            ...
            if (gradient < 0)
               zlist[[k]] <-
                  rev(ties (d = 1:length(zlist[[k]]), delta = rev(zlist[[k]])))
            if (gradient == 0)
               zlist[[k]] <- mean(zlist[[k]])
         }
      }
      for (k in 1:p)
      {
         labs <- names(zlist[[k]])
         scaling.factor <-
            as.numeric(sqrt(t(zlist[[k]]) %*% t(Gk[[k]]) %*% N.mat %*% Gk[[k]] %*% zlist[[k]]))
         if (is.na(1 / scaling.factor))
            scaling.factor <- 1
         scaling.vec <- rep(scaling.factor, length(zlist[[k]]))
         if (identical(factor.type[k], "nom"))
            zlist[[k]] <- as.vector(zlist[[k]] / scaling.vec)
         if (identical(factor.type[k], "ord"))
         {
            u.temp <- sum(diag(Lk.mat[[k]]))
            v.temp <- sum(zlist[[k]] * diag(Lk.mat[[k]]))
      ...
      while (again)
      {
         ...
         Yr <-
            scale(
               svd.out$u[, h.vects] %*% diag(svd.out$d[h.vects]) %*% t(svd.out$v[, h.vects]),
               scale = FALSE,
               center = TRUE
            )
         RSS <- sum((Hmat.cent - Yr) ^ 2)
         ...
         if (again) {
            for (k in 1:p) {
               ...
               if (identical(factor.type[k], "nom")) {
                  scale.factor <-
                     as.numeric(sqrt(t(Yr[, k]) %*% Gk[[k]] %*% solve(Lk.mat[[k]]) %*% t(Gk[[k]]) %*% Yr[, k]))
                  zlist[[k]] <-
                     as.vector((solve(Lk.mat[[k]]) %*% t(Gk[[k]]) %*% Yr[, k]) / scale.factor)
               }
               if (identical(factor.type[k], "ord")) {
                  zlist[[k]] <-
                     as.vector((solve(Lk.mat[[k]]) %*% t(Gk[[k]]) %*% Yr[, k]))
                  gradient <-
                     sign(lm(zlist[[k]] ~ I(1:length(zlist[[k]])))$coef[2])
                  if (gradient > 0)
                     zlist[[k]] <- ties (d = 1:length(zlist[[k]]), delta = zlist[[k]])
                  if (gradient < 0)
                     zlist[[k]] <-
      ...
      Hopt.full<-cbind(Hmat[,1:p],Xcont)
      
      Hopt.info <- biplot.check.X(Hopt.full, scaled.mat = F)
      Hopt<-Hopt.info$X
      
      unscaled.Hopt<-Hopt.info$unscaled.X
      means<-Hopt.info$means
      
      G <- biplot.check.G(G, nrow(Hopt))
      Nmat <- t(G) %*% G
      Hopt.bar <- solve(Nmat) %*% t(G) %*% Hopt
      
      
      SSP.T <- t(Hopt) %*% Hopt
      SSP.B <- t(Hopt.bar) %*% Nmat %*% Hopt.bar
      SSP.W <- SSP.T - SSP.B
      ...
      if (weightedCVA == "unweightedCent")
         Cmat <- diag(J) - matrix(1 / J, nrow = J, ncol = J)
      if (is.na(match(weightedCVA, c(
         "weighted", "unweightedI", "unweightedCent"
      ))))
         stop(" Argument 'weightedCVA' must be one of 'weighted','unweightedI','unweightedCent' ")
      svd.step2 <- svd(t(Lmat) %*% t(Hopt.bar) %*% Cmat %*% Hopt.bar %*% Lmat)
      Vmat <- svd.step2$v
      lambdamat <- diag(svd.step2$d)
      svd.2sided <- Eigen.twosided(t(Hopt.bar) %*% Cmat %*% Hopt.bar, Wmat)
      Mmat <- svd.2sided$W
      lambdamat.2sided <- svd.2sided$Lambda.mat
      ...
      Lz <- rep(NA, p)
      for (i in 1:p)
         Lz[i] <- sum(Lk.mat[[i]] %*% zlist[[i]])
      zLz <- rep(NA, p)
      for (i in 1:p)
         zLz[i] <- sum(t(zlist[[i]]) %*% Lk.mat[[i]] %*% zlist[[i]])
      
      #Adapted for CVA
      Vr <- Vmat[, e.vects, drop = F]
      Mr <- Mmat[, e.vects, drop = F]
      ...
      if (!is.null(Xcont))
         all.var.list <-
         vector("list", p + ncol(Xcont))
      else
         all.var.list <- vector("list", p)
      for (j in 1:p)
         all.var.list[[j]] <- zlist[[j]]
      if (!is.null(Xcont))
      ...
      reflect.mat <- diag(dim.biplot)
      if (reflect == "x" & dim.biplot < 3)
         reflect.mat[1, 1] <- -1
      if (reflect == "y" & dim.biplot == 2)
         reflect.mat[2, 2] <- -1
      if (reflect == "xy" &
          dim.biplot == 2)
         reflect.mat[1:2, 1:2] <- diag(-1, 2)
      rotate.mat <- diag(dim.biplot)
      if (dim.biplot == 2)
      {
         if (!is.null(rotate))
         {
            radns <- pi * rotate / 180
            rotate.mat <-
               matrix(c(cos(radns),-sin(radns), sin(radns), cos(radns)), ncol = 2)
         }
      }
      Z <- Z %*% rotate.mat %*% reflect.mat
      
      Z.means.mat <- solve(t(G) %*% G) %*% t(G) %*% Z
      Z.means.mat<-as.matrix(Z.means.mat)
      dimnames(Z) <- list(dimnames(X)[[1]], NULL)
      var.names <- c(dimnames(X)[[2]], dimnames(Xcont)[[2]])
      Vr.all <- solve(Mmat)[e.vects,,drop=F]
      
      if (!is.null(Xcont))
         num.vars <- p + ncol(Xcont)
      else
         num.vars <- p
      if (!is.null(Xcont))
         cont.var.names <- dimnames(Xcont)[[2]]
      
      ax.present <- ax.nominal.present <- ax.ordinal.present <- FALSE
      if (!is.null(Xcont))
      {
         ax <-
            do.call("biplot.ax.control", c(ncol(Xcont), list(cont.var.names), ax,tick.label.col="black",label.col = "black"))
         if (ax$type == "interpolation")
         {
            warning ("Only prediction biplot axes constructed")
            ax$type <- "prediction"
      ...
      {
         ax.nominal <-
            do.call("biplot.nominal.ax.control",
                    c(
                       sum(factor.type == "nom"),
      ...
      {
         ax.ordinal <-
            do.call("biplot.ordinal.ax.control",
                    c(
                       sum(factor.type == "ord"),
                       list(var.names[1:length(factor.type)][factor.type ==
                                                                "ord"]),
      ...
      if (!ax.present)
         ax <- NULL
      if (!ax.nominal.present)
         ax.nominal <- NULL
      if (!ax.ordinal.present)
         ax.ordinal <- NULL
      
      axes.direction <-
         solve(diag(diag(t(Vr.all) %*% Vr.all))) %*% t(Vr.all) %*% rotate.mat %*% reflect.mat
      axes.direction.nominal <-
         axes.direction[(1:length(factor.type))[factor.type == "nom"], ]
      axes.direction.ordinal <-
         axes.direction[(1:length(factor.type))[factor.type == "ord"], ]
      axes.direction <- axes.direction[-(1:length(factor.type)), ]
      nominal.var.list <-
         all.var.list[(1:length(factor.type))[factor.type == "nom"]]
      ordinal.var.list <-
         all.var.list[(1:length(factor.type))[factor.type == "ord"]]
      all.var.list <- all.var.list[-(1:length(factor.type))]
      if (!is.null(ax))
      {
         z.axes <- vector("list", length(ax$which))
         for (j in 1:length(ax$which))
            z.axes[[j]] <-
               calibrate.cat.axis(
                  ...
                  ax$orthogy,
                  ax$oblique,
                  markers = all.var.list[[ax$which[j]]],
                  labels = names(all.var.list[[ax$which[j]]])
               )
      }
      else
         z.axes <- NULL
         ...
         for (j in 1:length(ax.nominal$which))
            z.axes.nominal[[j]] <-
               calibrate.cat.axis(
                  ...
                  ax.nominal$orthogy,
                  ax.nominal$oblique,
                  markers = nominal.var.list[[ax.nominal$which[j]]],
                  labels = names(nominal.var.list[[ax.nominal$which[j]]])
               )
      }
      else
         z.axes.nominal <- NULL
      ...
      if (!is.null(ax.ordinal))
      {
         ...
         for (j in 1:length(ax.ordinal$which))
         {
            ...
            while (i < length(current.markers) - 1)
            {
               if (abs(current.markers[i] - current.markers[i + 1]) < 1.5 * .Machine$double.eps)
               {
                  names(current.markers)[i] <-
                     paste(names(current.markers)[i],
                           names(current.markers)[i + 1],
                           sep = "*")
                  current.markers <- current.markers[-(i + 1)]
               }
               else
      ...
      alpha.bags <-
         do.call("biplot.alpha.bag.control", c(J, list(dimnames(G)[[2]]), alpha.bags))
      z.bags <- vector("list", length(alpha.bags$which))
      if (length(alpha.bags$which) > 0)
         for (j in 1:length(alpha.bags$which))
         {
            ...
            if (dim.biplot == 2)
               ...
               calc.alpha.bags(
                  mat,
                  aa = alpha.bags$alpha[j],
                  na.rm = TRUE,
                  approx.limit = alpha.bags$max[j]
               )
            if (dim.biplot == 1)
               z.bags[[j]] <-
               quantile(mat, c((100 - alpha.bags$alpha[j]) / 200,
                               1 - (100 - alpha.bags$alpha[j]) / 200,
                               0.5
               ))
         }
      kappa.ellipse <-
         do.call("biplot.kappa.ellipse.control",
                 c(J, list(dimnames(G)[[2]]), dim.biplot, kappa.ellipse))
      z.ellipse <- vector("list", length(kappa.ellipse$which))
      if (length(kappa.ellipse$which) > 0)
         for (j in 1:length(kappa.ellipse$which))
         {
            ...
            if (dim.biplot == 2)
               z.ellipse[[j]] <-
               calc.concentration.ellipse(mat, kappa.ellipse$kappa[j])
            if (dim.biplot == 1)
               z.ellipse[[j]] <-
               qnorm(c(
                  1 - pnorm(kappa.ellipse$kappa[j]),
                  pnorm(kappa.ellipse$kappa[j])
               ), mean(mat), sqrt(var(mat)))
            if (dim.biplot == 3) {
               require(rgl)
               z.ellipse[[j]] <-
                  ellipse3d(
                     x = var(mat),
                     centre = apply(mat, 2, mean),
                     t = kappa.ellipse$kappa[j]
                  )
            }
         }
      if (dim.biplot == 1)
      {
         density.style <-
            do.call("biplot.density.1D.control", c(J, list(dimnames(G)[[2]]), density.style))
         z.density <- vector("list", length(density.style$which))
         if (length(density.style$which) > 0)
            for (j in 1:length(density.style$which))
            {
               ...
               mat <- Z[G[, class.num] == 1,]
               z.density[[j]] <-
                  density(mat, bw = density.style$bw[j], kernel = density.style$kernel[j])
            }
      }
      if (dim.biplot == 2)
      {
         density.style <-
            do.call("biplot.density.2D.control", c(J, list(dimnames(G)[[2]]), density.style))
         if (!is.null(density.style$which))
         {
            if (density.style$which == 0)
               mat <- Z
            ...
            if (is.null(density.style$h))
               z.density <-
               kde2d(mat[, 1], mat[, 2], n = density.style$n, lims = c(xlim, ylim))
            else
               z.density <-
               kde2d(
                  mat[, 1],
      ...
      if (dim.biplot == 3)
      {
         density.style <-
            do.call("biplot.density.2D.control", c(J, list(dimnames(G)[[2]]), density.style))
         if (!is.null(density.style$which))
            warning("No density plots in 3D")
      }
      ...
      class.means <-
         do.call("biplot.mean.control", c(J, list(dimnames(G)[[2]]), class.means))
      ...
      if (dim.biplot == 2 & !is.null(prior.p))
      {
         ...
         # Z.region <- biplot.create.regions2(S.svd$v[,e.vects[1:dim.biplot]], plot.range=plot.range, plot=F, plot.coords=plot.coords,
         #                                    region.style=class.regions,region.mid = Z.means.mat)
